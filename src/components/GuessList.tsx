...
export default function GuessList({ guesses }: GuessListProps) {
  ...
  const firstEmptyIndex = rows.findIndex((r) => !r);
  return (
    <div className="overflow-x-auto w-full">
      ...
      > */}
        <div
          className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 md:gap-3 text-xs sm:text-sm md:text-base border-b border-dashed mb-4 pb-2"
          role="row"
          aria-label="Column Labels for the Guesses"
        >
          <label
        ...
        {rows.map((guess, i) =>
          guess ? (
            ...
            <div
              key={i}
              className={`empty-row grid grid-cols-1 md:gap-3 border border-neutral-700 rounded-xl mb-2 
                ${i === firstEmptyIndex ? "bg-neutral-600/80" : "bg-neutral-700/50"}`}
              role="row"
            >
              <span className="col-span-1 text-center rounded-sm text-neutral-50 font-bold text-xl p-3">
                {i + 1}
              </span>
            </div>
